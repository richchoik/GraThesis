        ...
        private void OnBeginRequest(object sender, EventArgs e)
        {
            ...
            if (request.HttpMethod == "POST" && request.ContentType.Contains("application/json"))
            {
                try
                {
                    ...
                    // Nếu middleware bật, kiểm tra và chặn payload đáng ngờ
                    if (enableMiddleware && IsPayloadSuspicious(jsonPayload))
                    {
                    ...
                    // Nếu sandbox bật, xử lý payload qua sandbox
                    if (enableSandbox)
                    {
                    ...
                    // Lưu payload đã xử lý để controller sử dụng
